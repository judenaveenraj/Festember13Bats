...
Jasper.BehaviorManager = function () {
    ...
    this._name_beh = {
        ...
        'lines': Jasper.LinesDrawBehavior,
        'curves': Jasper.CurvesDrawBehavior
    };
        ...
        if (typeof (behaviorName) == "string") {
            if (hasOwnProperty(this._name_beh, behaviorName)) {
                ...
                if(this._beh_attrs[behaviorName] !== undefined){
                    Object.extend(behavior, this._getCopyObject(this._beh_attrs[behaviorName]));//this._beh_attrs[behaviorName]);
                }
    ...
    _getCopyObject: function (obj) {
                ...
                if (obj instanceof Array) {
                    ...
                    for (var i = 0, len = obj.length; i < len; i++) {
                        copy[i] = this._getCopyObject(obj[i]);
                    }
                    return copy;
                }
                ...
                if (obj instanceof Object) {
                    var copy = {};
                    for (var attr in obj) {
                        if (obj.hasOwnProperty(attr)) copy[attr] = this._getCopyObject(obj[attr]);
                    }
                    return copy;
                }
...
Jasper.CollisionManager.prototype = {
        ...
        if(this._layerNum_QuadTree[obj.getLayer().getLayerNumber()] === undefined){
            ...
            args = {
               // mandatory fields
                x:0,
                y:0,
                w:Jasper._core.activeScene.worldW,//obj.getLayer().getWorldSize().x,
                h:Jasper._core.activeScene.worldH,//obj.getLayer().getWorldSize().y,       
            };
    ...
    _activateCollisionsInLayer: function(layerNum){
        ...
        var mousePos = Jasper._mouseManager.getMousePos();
        var search = {
            x: mousePos[0],
            y: mousePos[1],
            w: 1,
        ...
        var l = collidedObjs.length;
        for(var i=0; i<l; i++){
            if(search != collidedObjs[i]){

                if  (search.x < (collidedObjs[i].x + collidedObjs[i].w) && 
                    (search.x+search.w) > collidedObjs[i].x && 
                    search.y < (collidedObjs[i].y + collidedObjs[i].h) && 
                    (search.y+search.h) > collidedObjs[i].y){
                    if(collidedObjs[i].obj.getBehavior("mouse") !== undefined)
                        collidedObjs[i].obj.getBehavior("mouse").onMouseOver(mousePos[0],mousePos[1]);
                }
            }
...
Jasper.Core    = function(args){
    ...
    this.testcanvas = undefined;
    this.testCtx = undefined;
    //this. core;
...
Jasper.Core.prototype = {
        ...
        _createCanvas: function(){
            ...
            this.canvas.ontouchmove = function(e){ Jasper._mouseManager.mouseMove(e);};
            ...
            this.testcanvas.id = "pixelperfect";
            this.testcanvas.width = this.canvasWidth ;
            this.testcanvas.height = this.canvasHeight ;
            this.testCtx = this.testcanvas.getContext('2d');
            //document.getElementById(this.containerId).appendChild(this.testcanvas);
            //
        },

        //TESTS
        _getTestCanvas: function(){
            return this.testCtx;
        },
        //
...
Jasper.Object.prototype = {
        ...
        getRendererBehavior: function(){
            return this._rendererBehavior;
        },
...
Jasper.CurvesDrawBehavior = function(){
    this.strokeColor = 'black';
    this.strokeWidth = 5;
    this.points = [];
    this.stroke = true;
...
Jasper.CurvesDrawBehavior.prototype = new Jasper.RenderableBehavior();

Object.extend(Jasper.CurvesDrawBehavior.prototype, {
        _attr: function(args){
            if(args.strokeColor !== undefined)
                this.setStrokeColor(args.strokeColor);
            if(args.strokeWidth !== undefined)
        ...
        render:function(ctx){
            ...
            if(l > 2){
               ...
               for (i = 1; i < this.points.length - 2; i ++)
               {
                  ...
                  ctx.quadraticCurveTo(this.points[i][0], this.points[i][1], xc, yc);
               }
             // curve through the last two this.points
             ctx.quadraticCurveTo(this.points[i][0], this.points[i][1], this.points[i+1][0],this.points[i+1][1]);
            }
        ...
        popLastPoint: function(){
            var p = this.points.pop();
            return p ;  
        },
        popFirstPoint: function(){
...
Jasper.LinesDrawBehavior = function(){
    this.strokeColor = 'black';
    this.strokeWidth = 5;
    this.points = [];
    this.stroke = true;
...
Jasper.LinesDrawBehavior.prototype = new Jasper.RenderableBehavior();

Object.extend(Jasper.LinesDrawBehavior.prototype, {
        _attr: function(args){
            if(args.strokeColor !== undefined)
                this.setStrokeColor(args.strokeColor);
            if(args.strokeWidth !== undefined)
...
Object.extend(Jasper.MouseBehavior.prototype,{
        _attr: function(args){
            ...
            if(args.onMouseOver !== undefined)
                this.setOnMouseOver(args.onMouseOver);
        },
        _init: function(object){
        ...
        setOnMouseOver: function(mouseOver){
